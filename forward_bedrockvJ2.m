...
%     burial = interp1(mT,mz,time); %Depths at times in 'time' %old
    ...
    for kk=(nt-1):-1:1
        ...
        P10z_spal = pf*P10spal*exp(-CNprop.rho*100*bz/CNprop.Lspal);
        P10z_nmc = pf*P10nmc*exp(-CNprop.rho*100*bz/CNprop.Lnmc);
        P10z_fm = pf*P10fm*exp(-CNprop.rho*100*bz/CNprop.Lfm);
        P26z_spal = pf*P26spal*exp(-CNprop.rho*100*bz/CNprop.Lspal);
        P26z_nmc = pf*P26nmc*exp(-CNprop.rho*100*bz/CNprop.Lnmc);
        P26z_fm = pf*P26fm*exp(-CNprop.rho*100*bz/CNprop.Lfm);
        
              
        N10(:,kk) = N10(:,kk+1)*exp(-dt*CNprop.lambda_Be); % changed '(kk)' to '(:,kk+1)'

        ff = CNprop.lambda_Be+CNprop.rho*erate/CNprop.Lspal;
        N10(:,kk) = N10(:,kk) + P10z_spal*(1.0-exp(-ff*dt))/ff; % changed '(kk)' to '(:,kk)'

        ff = CNprop.lambda_Be+CNprop.rho*erate/CNprop.Lnmc;
        N10(:,kk) = N10(:,kk) + P10z_nmc*(1.0-exp(-ff*dt))/ff; % changed '(kk)' to '(:,kk)'
        
        ff = CNprop.lambda_Be+CNprop.rho*erate/CNprop.Lfm;
        N10(:,kk) = N10(:,kk) + P10z_fm*(1.0-exp(-ff*dt))/ff; % changed '(kk)' to '(:,kk)'
        
        N26(:,kk) = N26(:,kk+1)*exp(-dt*CNprop.lambda_Al); % changed '(kk)' to '(:,kk+1)'
        
        ff = CNprop.lambda_Al+CNprop.rho*erate/CNprop.Lspal;
        N26(:,kk) = N26(:,kk) + P26z_spal*(1.0-exp(-ff*dt))/ff; % changed '(kk)' to '(:,kk)'
        
        ff = CNprop.lambda_Al+CNprop.rho*erate/CNprop.Lnmc;
        N26(:,kk) = N26(:,kk) + P26z_nmc*(1.0-exp(-ff*dt))/ff; % changed '(kk)' to '(:,kk)'
        
        ff = CNprop.lambda_Al+CNprop.rho*erate/CNprop.Lfm;
